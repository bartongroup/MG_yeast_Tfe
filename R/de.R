...
edger_de <- function(set, gene_info, ctrs = NULL, normfac = NULL, id = "gene_id", fdr_limit = 0.05, logfc_limit = 0) {
  genes <- gene_info %>% 
    select(gene_id, gene_name, gene_biotype, description) %>% 
    distinct()
...
edger_deseq_merge <- function(ed, de) {
  eds <- ed %>% 
    select(gene_id, logFC, PValue, FDR, contrast, gene_name) %>% 
    mutate(tool = "edgeR")
  des <- de %>% 
    select(gene_id, logFC, PValue, FDR, contrast, gene_name) %>% 
    mutate(tool = "DESeq2")
